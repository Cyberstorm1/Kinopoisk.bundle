...
class QtParser():
    ...
    def urlopen(self, obj):
        try:
            return urllib2.urlopen(obj, timeout=10, context = ssl.SSLContext(ssl.PROTOCOL_TLSv1))
        except urllib2.URLError, e:
            Log.Warn("Timeout for open trailer url (%s)" % obj)
            Log.Warn(e.args)
            print 'Timeout'
            ...
            if 'sample_description_table' in atom:
                for smtbl in atom['sample_description_table']:
                    if 'data_format' in smtbl:
                        if smtbl['data_format'] in self.video_formats and (smtbl['data_format'].strip() != 'raw' or \
                                                                        (smtbl.get('audio_channels', 0) == 0)):
                            is_video = True
    ...
    def parseatom_stsd(self, atom_structure, atomname, atomsize, atomdata, baseoffset):
        ...
        while i < atom_structure['number_entries']:
            ...
            if smp_tbl['encoder_vendor'] == '\x00\x00\x00\x00':
                ...
                if smp_tbl['data_format'] in self.video_formats and (smp_tbl['data_format'].strip() != 'raw' or \
                                                                    (smp_tbl.get('audio_channels', 0) == 0)):
                    self.info['fileformat'] = 'mp4'
